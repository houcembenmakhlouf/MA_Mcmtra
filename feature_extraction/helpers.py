...
class MMDataset(Dataset):
    def __init__(
        self, dloc, img_transform=None, txt_transform=None, txt_processor=None
    ):
        self.file_names = pd.read_csv(
            os.path.join(dloc, "valid_pairlist.txt"), header=None
        )
        self.dloc = dloc
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(os.path.join(self.dloc, "images", fname + ".jpg")).convert(
            "RGB"
        )
        text = (
            open(
                os.path.join(self.dloc, "texts", fname + ".txt"),
                "r",
                encoding="utf-8",
                errors="ignore",
            )
...
def process_tweet(tweet, text_processor):
    ...
    clean_tweet = [
        word.strip() for word in proc_tweet if not re.search(r"[^a-z0-9.,\s]+", word)
    ]
...
def get_bert_embeddings(tweet, model, tokenizer, device):
    ...
    input_ids = torch.tensor([tokenizer.encode(tweet, add_special_tokens=True)]).to(
        device
    )
    ...
    # sent_emb_last = last_out[:, 0, :].cpu().numpy()
    # sent_emb_last = np.squeeze(sent_emb_last)
