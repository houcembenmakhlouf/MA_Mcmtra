...
class HBMNetAttLike(nn.Module):
    def __init__(self, vdim, tdim, nclasses, device):
        super(HBMNetAttLike, self).__init__()
        self.device = device
        ...
        self.reply_lfc1 = nn.Linear(1, 1)
        self.reply_lbn1 = nn.BatchNorm1d(1)
        self.reply_ldp1 = nn.Dropout(0.5)

        self.reply_fc2 = nn.Linear(257, 128)
        ...
        self.quote_lfc1 = nn.Linear(1, 1)
        self.quote_lbn1 = nn.BatchNorm1d(1)
        self.quote_ldp1 = nn.Dropout(0.5)

        self.quote_fc2 = nn.Linear(257, 128)
    ...
    def forward(self, vfeat, tfeat, lfeat):
        x1 = self.tweet_vdp1(
        ...
        for i, _ in enumerate(tfeat["replies"]):
            ...
            x3 = self.reply_ldp1(
                self.relu(
                    self.reply_lbn1(
                        self.reply_lfc1(lfeat["replies"][i].to(self.device))
                    )
                )
            )
            x4 = torch.cat((x1, x2, x3), axis=1)
            x4 = self.reply_dp2(self.relu(self.reply_bn2(self.reply_fc2(x4))))
            w = tfeat["replies_indices"][:, i].unsqueeze(dim=1).to(self.device)
            x4 = x4 * w
            reply_x_list.append(x4)
        ...
        for i, _ in enumerate(tfeat["quotes"]):
            ...
            x3 = self.quote_ldp1(
                self.relu(
                    self.quote_lbn1(self.quote_lfc1(lfeat["quotes"][i].to(self.device)))
                )
            )
            x4 = torch.cat((x1, x2, x3), axis=1)
            x4 = self.quote_dp2(self.relu(self.quote_bn2(self.quote_fc2(x4))))
            w = tfeat["quotes_indices"][:, i].unsqueeze(dim=1).to(self.device)
            x4 = x4 * w
            quote_x_list.append(x4)
