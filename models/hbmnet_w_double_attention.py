...
class HBMNetDoubleAtt(nn.Module):
    ...
    def forward(self, vfeat, tfeat):
        ...
        for i, _ in enumerate(tfeat["replies"]):
            ...
            x3, weights = self.reply_self_att(
        ...
        for i, _ in enumerate(tfeat["quotes"]):
            ...
            x3, weights = self.quote_self_att(
                query=x3.unsqueeze(dim=1), context=x3.unsqueeze(dim=1)
            )
            x3 = x3.squeeze()
